...
public class StoreAdapter extends RecyclerView.Adapter<StoreAdapter.MyViewHolder> {
    ...
    public StoreAdapter(Fragment fragment, RecyclerView recyclerView, List<BeanStore> map) {
    ...
    @Override
    public void onBindViewHolder(@NonNull final StoreAdapter.MyViewHolder holder, int position) {
        try {
            final BeanStore beanStore = map.get(position);
            if (beanStore != null) {
                holder.txtShopName.setText(beanStore.getShop_name());
                holder.txtPhone.setText(beanStore.getPhone());
                holder.txtAddress.setText(beanStore.getAddress());

                holder.txtID.setText(beanStore.getId() + "");
...
//                        FragmentHelper.add(DetailOrderFragment.newInstance(1));
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder {
        TextView txtID, txtShopName, txtAddress, txtPhone;

        public MyViewHolder(View itemView) {
            super(itemView);
            txtID = itemView.findViewById(R.id.idshop);
            txtShopName = itemView.findViewById(R.id.nameshop);
